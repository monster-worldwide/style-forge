import { getCamelCase, indentedLine, getDistinctFills } from '../../../utils';
import { FileDescription, IconDefinition, IconObject } from '../../../types';
import { DOMParser } from 'xmldom';

export const createIconsCjs = (data: IconObject) => {
  ...
  Object.keys(data).forEach((key) => {
    ...
    if (!icon.data) {
      ...
      result.push({
        path: `dist/icons-cjs/${icon.fileName}.js`,
        content: createIconCjs(icon),
      });
    }
  });
  return result;
 ...
 */
const createIconCjs = (icon: IconDefinition) => {
  let content = generateInteropFlag();
  content += generateExport(icon.componentName);
...
const generateExport = (name: string) => {
  return indentedLine(
...
const generateIconComponent = (icon: IconDefinition) => {
  ...
  let content = indentedLine(
    `const ${icon.componentName} = React.forwardRef(function (_ref, ref) {`,
  );
